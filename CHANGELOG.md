...
- **Per-glyph cloud diffusion replaces the global scrim.** The full-screen surface-tint scrim is gone — bg stays raw past ~10 px from any letter. Each glyph carries a Gaussian halo of `ColorScheme.surface` via `TextStyle.shadows` (`blurRadius: 3`), giving an alpha falloff of `exp(-d² / 18)` at distance `d` (px) from the glyph: ~95% at 1 px, ~61% at 3 px, ~41% at 4 px, ~0% past 10 px. Applied to `HomePage`'s AppBar title and `TodoTile`'s body text — both the static `RainbowStrikethrough` rendering and the inline edit `TextField` pick up the shadow because they share `baseStyle`.
