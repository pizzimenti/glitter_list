...
- **Frosted-glass strip behind text replaces the global scrim.** The full-screen surface-tint scrim is gone, and the earlier `TextStyle.shadows` cloud is replaced too — that approach could only *tint* (paint a colored, blurred glyph silhouette) rather than actually blurring the bg pixels. Now a `ClipRect` + `BackdropFilter(ImageFilter.blur(sigmaX: 10, sigmaY: 10))` wraps the AppBar title's `Text` and the `TodoTile`'s `title` widget (same wrapper for both the static `RainbowStrikethrough` and the inline edit `TextField`), giving an Aero-style rectangular frosted strip per text widget — bg is genuinely Gaussian-blurred under the rect, not just colored over.
