...
- **Stronger diffusion scrim.** Bumped the scrim alpha from 0.3 to 0.5 — the glitter is still visible, but the lilac text now reads more comfortably on the brighter pink background. Scrim is now a separate `ColoredBox` layer above the image rather than a `DecorationImage.colorFilter`, so it composites after the saturation boost.
