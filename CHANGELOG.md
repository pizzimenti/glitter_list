...
- **iOS CocoaPods integration committed.** `ios/Podfile`, `Podfile.lock`, the pod xcconfig includes, workspace reference, and frameworks build phases generated by `pod install` during the first physical-device deploy. Required scaffolding for any iOS archive build; also carries the `DEVELOPMENT_TEAM` for signing.

### Changed

- **iPhone-only device family.** `TARGETED_DEVICE_FAMILY` narrowed from `"1,2"` to `1` across all build configs. iPad support deferred — under the iOS 26 SDK, iPhone-only apps run in a resizable window on iPad instead of letterboxing, so nothing degrades there; revisit if/when iPad screenshots + review are worth the submission overhead.
- **Encryption export compliance declared.** `ITSAppUsesNonExemptEncryption = false` in `ios/Runner/Info.plist` — the app uses only OS-provided (exempt) encryption, and declaring it skips the per-upload compliance questionnaire in App Store Connect.

### Notes

- App Store submission readiness: since 2026-04-28 Apple requires uploads built with the iOS 26 SDK (Xcode 26), which needs macOS Sequoia 15.6+. No repo change can satisfy that — it's a build-machine constraint.
