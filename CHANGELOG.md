...
- **True frosted-glass cloud via `CloudBlur` custom render object.** The shadow-stack approach could only tint (paint a colored copy of the glyph silhouette behind the text), not actually blur bg pixels. `CloudBlur` (`lib/ui/cloud_blur.dart`) is a two-child `RenderBox` that paints `bg` sharply, then a Gaussian-blurred copy of `bg` masked by the alpha of `foreground` (also Gaussian-blurred outward to soften the cloud edge), then `foreground` sharply on top. Compositing uses `Canvas.saveLayer` with `BlendMode.dstIn` and `ImageFilter.blur` — bg pixels behind text and chrome are genuinely blurred, falling off Gaussianly into the raw bg around them. `bgBlurSigma: 14` (bg fuzziness) and `maskBlurSigma: 8` (cloud spread) are the two knobs.
